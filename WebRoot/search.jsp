...
if(state != null && state.trim().equals("adminLogined")) {
	isAdmin = true;
}
...
<div class="wrap">
  ...
  <div class="content">
  	...
  	<form action="searchAnalysis.jsp" method="post">
    ...
    <table class="table_list parent_forum " cellspacing="0" cellpadding="0" width="100%" border="0">
      ...
      <tbody>
        ...
        String from = url + queryStr;
        for(int i=0; i<articles.size(); i++) {
        	Article a = articles.get(i);        	
        %>
        <tr class="<%= a.isSingular() ? "zebra" : ""%>">
          <td class="title"><strong class="green">？</strong> <a href="searchDetail.jsp?id=<%= a.getId() %>" target="_blank" title="<%= a.getTitle()%>"><%= a.getTitle() %></a> <span class="forum_link">[<span class="parent"><a href="http://bbs.csdn.net/forums/Java">Java</a></span> <a href="http://bbs.csdn.net/forums/Java_WebDevelop">Web 开发</a>]</span></td>
          <td class="tc">50</td>
          ...
          <td class="tc"><a href="http://my.csdn.net/qq_32461501" rel="nofollow" target="_blank" title="qq_32461501">qq_32461501</a><br>
            ...
            %>
          		<td class="tc">
          		<a href="panelDelete.jsp?from=<%= from %>&id=<%= a.getId() %>" target="_blank" onClick="javascript:return confirm('该删除操作将无法恢复！是否继续？')">删除</a>
          		<br>
          		<a href="panelModify.jsp?from=<%= from %>&id=<%= a.getId() %>" target="_blank" >修改</a>
          		</td>
            <%
            }
            %>
